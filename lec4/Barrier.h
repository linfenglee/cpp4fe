...
class DoubleBarrier : public Barrier
{
 public:
  DoubleBarrier(double downLevel, double upLevel)
     :downBarrierLevel(downLevel), upBarrierLevel(upLevel){}
  virtual double BarrierPayoff(double S, double t, double continuation) const
  {
    return ((S<=downBarrierLevel) || (S>=upBarrierLevel)) ? 0 : continuation;
  }
 protected:
  virtual Barrier* Clone() const
  {
    return (new DoubleBarrier(*this));
  }
 private:
  double downBarrierLevel;
  double upBarrierLevel;
