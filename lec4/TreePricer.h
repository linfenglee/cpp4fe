...
//class AdjustedBinomialTreePricer : public BinomialTreePricer
//{
// public:
//  AdjustedBinomialTreePricer(int N) : BinomialTreePricer(N){}
// protected:
//  virtual void ModelSetup(double S0, double sigma, double rate, double dt)
//  {
//    double anchor = trade.Anchor();
//    double xi = std::pow(anchor/S0, 2/(N + 1));
//    double b = std::exp((2*rate+sigma*sigma)*dt)+ xi;
//    u = (b + std::sqrt(b*b - 4*xi*std::exp(2*rate*dt))) / 2 / std::exp(rate*dt);
//    p = (std::exp(rate*dt) -  xi/u) / (u - xi/u);
//    currentSpot = S0;
//  }
//  virtual double GetSpot(int ti, int si) const
//  {
